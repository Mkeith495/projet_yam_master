...
const GAME_INIT = {
  gameState: {
    ...
    player1Score: 0,
    player2Score: 0,
    player1Tokens: 12, // Ajout des pions restants
    player2Tokens: 12, // Ajout des pions restants
    choices: {},
    deck: {},
  },
};
...
const GameService = {
  init: {
    gameState: () => {
      ...
      game["gameState"] = { ...GAME_INIT.gameState }; // On spread pour éviter les références partagées
      game["gameState"]["timer"] = TURN_DURATION;
      game["gameState"]["deck"] = { ...DECK_INIT };
      game["gameState"]["choices"] = { ...CHOICES_INIT };
      game["gameState"]["grid"] = [...GRID_INIT];
      game["gameState"]["grid"] = JSON.parse(JSON.stringify(GRID_INIT)); // Deep copy de la grille
      return game;
    },
  ...
  send: {
    forPlayer: {
      // Dans game.service.js -> GameService.send.forPlayer
        ...
        return {
          ...
          idOpponent:
            playerKey === "player:1"
              ? game.player2Socket.id
              : game.player1Socket.id,
          // On ajoute les infos de score initiales
          gameState: {
            player1Score: game.gameState.player1Score,
            player2Score: game.gameState.player2Score,
            player1Tokens: game.gameState.player1Tokens,
            player2Tokens: game.gameState.player2Tokens,
          },
        };
      },
      ...
      // À ajouter dans game.service.js -> GameService.send.forPlayer
  ...
  grid: {
    ...
    // À rajouter dans GameService.grid :

    calculateScore: (grid, playerKey) => {
      let score = 0;
      const size = 5;

      // Fonction utilitaire pour vérifier les segments
      const checkLine = (cells) => {
        ...
        for (let cell of cells) {
          if (cell.owner === playerKey) {
            ...
            if (count === 3) lineScore += 1;
            if (count === 4) lineScore += 2;
            count = 0;
          }
        }
        if (count === 3) lineScore += 1;
    ...
    // Vérification si un alignement de 5 existe (Victoire instantanée)
    checkWinConditionFive: (grid, playerKey) => {
      const size = 5;
      // ... même logique que calculateScore mais retourne true si count === 5
      const isFive = (cells) => {
        let count = 0;
        for (let cell of cells) {
          if (cell.owner === playerKey) {
      ...
      const updatedGrid = grid.map((row, rowIndexParsing) =>
        row.map((cell, cellIndexParsing) => {
          if (
            ...
            // On décrémente les pions du joueur actuel
            if (currentTurn === "player:1") gameState.player1Tokens--;
            else gameState.player2Tokens--;
